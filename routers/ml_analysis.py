...
async def get_risk_analysis(
    ...
    risk_analysis = calculate_patient_risk(db, current_user.id) or []

    # Estatísticas resumidas com padronização
    total_patients = len(risk_analysis)
    high_risk = len([p for p in risk_analysis if p.get("risk", "").lower() == "alto"])
    moderate_risk = len([p for p in risk_analysis if p.get("risk", "").lower() == "médio"])
    low_risk = len([p for p in risk_analysis if p.get("risk", "").lower() == "baixo"])
...
async def get_patient_risk_details(
    ...
    """
    Detalhes da análise de risco de um paciente específico
    """
    if current_user.type != UserType.PSICOLOGO:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
