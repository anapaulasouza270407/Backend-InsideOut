...
from services.email_service import send_email_request_accepted, send_email_request_reject
...
async def update_request_status(
    ...
    #-----------------------------------------
    # ENVIA E-MAIL SE A SOLICITAÇÃO FOR ACEITA
    #-----------------------------------------
    if update_data.status == "ACEITO":
        send_email_request_accepted(
            patient_email=request.patient_email,
            patient_name=request.patient_name,
            psychologist_name=current_user.name
        )
    if update_data.status == "REJEITADO":
